...
export default function OfflineBar() {
  ...
  // «Тик» раз в минуту: заставляет React перерисовать полоску, чтобы isStale
  // пересчиталось со временем для долго открытой вкладки (WR-01).
  ...
  useEffect(() => {
    ...
    // Раз в минуту: перечитываем метку (фоновая синхронизация могла обновить
    // время уже после монтирования — WR-02) и инкрементируем тик, чтобы React
    // перерисовал полоску и пересчитал «устаревшесть» данных (WR-01).
    const id = setInterval(() => {
      void load();
      setTick((t) => t + 1);
    }, 60_000);
    return () => clearInterval(id);
  }, []); // подписка ставится один раз при монтировании
