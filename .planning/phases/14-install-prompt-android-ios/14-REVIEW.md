...
findings:
  ...
  total: 9
status: resolved
resolution: "CR-01, WR-01, WR-02, WR-03 исправлены (коммит e7aff75); WR-04/WR-05 приняты как есть по решению D-03"
---
...
**Глубина:** standard
**Файлов проверено:** 6
**Статус:** проблемы устранены (см. раздел «Устранение» ниже)

## Устранение (2026-06-13, коммит e7aff75)

- **CR-01** ✓ исправлено — добавлены keyframes `slideUp` в `app/globals.css`; анимация iOS-шторки заработала.
- **WR-01** ✓ исправлено — баннер/шторка установки подняты на `z-[60]` над плавающей кнопкой Telegram.
- **WR-02** ✓ исправлено — на Android кнопка «Установить приложение» в настройках показывается только при `canPromptAndroid` (нет кнопки-пустышки).
- **WR-03** ✓ исправлено — переписан вводящий в заблуждение комментарий в `openFromSettings`.
- **WR-04 / WR-05** — приняты как есть: простая эвристика детекта iPhone сознательно выбрана в решении D-03 (цель — телефон агента, не iPad/иные браузеры iOS).
- **Info-находки** — оставлены на усмотрение, не блокируют.

Сборка `npm run build` после правок — без ошибок типов.
