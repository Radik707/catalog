...
def log_visible_metric(rows: list[list]) -> int:
    """Посчитать и залогировать, сколько товаров реально ДОЙДЁТ до витрины.

    Витрина прячет остаток <= 1 (CatalogView/nav.ts) и товары с флагом «Скрыт».
    Метрика «сколько строк записано» этого не видит: прайс, у которого не прочиталась
    колонка «Остаток», даёт полный лист и почти пустую витрину. Случай 09.2026:
    631 строка записана, а на витрине осталось 115 товаров (только блок Акконда —
    единственный со складом «Основной»), и загрузчик отрапортовал «успех».

    Зовётся и при реальной записи, и в --dry-run: предпросмотр обязан показывать
    то же число, что получится после заливки. Возвращает число видимых товаров.
    """
    data = rows[1:]  # без строки заголовка
    num_products = len(data)
    visible = sum(
        1 for r in data
        if cell_stock(r[2] if len(r) > 2 else None) > 1
        and str(r[11] if len(r) > 11 else "") != "1"
    )
...
def upload_to_google_sheet(rows: list[list], num_files: int) -> None:
    ...
    # Сколько товаров реально дойдёт до витрины (см. log_visible_metric).
    log_visible_metric(rows)
...
def main():
    ...
    if args.dry_run:
        ...
        # Главная проверка предпросмотра: сколько товаров увидит клиент.
        log_visible_metric(rows)
