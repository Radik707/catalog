...
- Полное покрытие всех категорий из category_map.json (D-12)
  (на момент написания — 121: 117 исходных + 4 добавленных владельцем на чекпоинте этапа 5)
...
def test_порядок_разделов(structure_map):
    """Порядок разделов верхнего уровня — новый порядок владельца (лето: напитки первыми)."""
    expected = [
        "Напитки",
        "Снэки и прикассовое",
        "Сладкое",
...
def test_порядок_подгрупп_напитки(structure_map):
    """«Напитки» = только готовые напитки: воды/газировки → соки → энергетики (чай/кофе — в бакалее)."""
    expected = ["Воды и газировки", "Соки", "Энергетики"]
    actual = list(structure_map.get("Напитки", {}).keys())
    assert actual == expected, (
        f"Порядок подгрупп «Напитки» не совпадает:\n  Ожидали:  {expected}\n  Получили: {actual}"
    )


def test_чай_кофе_в_бакалее(structure_map):
    """Чай и кофе — сухая бакалея: подгруппа «Чай и кофе» в разделе «Крупы, лапша, бакалея»."""
    krupy = structure_map.get("Крупы, лапша, бакалея", {})
    assert "Чай и кофе" in krupy, "Подгруппа «Чай и кофе» должна быть в разделе «Крупы, лапша, бакалея»"
    assert "Чай и кофе" not in structure_map.get("Напитки", {}), "«Чай и кофе» не должно остаться в «Напитки»"


def test_порядок_подгрупп_сладкое(structure_map):
    """Порядок подгрупп в «Сладкое»: печенье/вафли → конфеты → шоколад/батончики → детское (порядок владельца)."""
    expected = ["Печенье и вафли", "Конфеты", "Шоколад и батончики", "Детское"]
    actual = list(structure_map.get("Сладкое", {}).keys())
    assert actual == expected, (
        f"Порядок подгрупп «Сладкое» не совпадает:\n  Ожидали:  {expected}\n  Получили: {actual}"
    )


def test_конфеты_начинаются_с_акконда(structure_map):
    """В «Конфеты» сначала Акконд и коробочные/наборы, потом весовые и прочее (порядок владельца)."""
    confety = structure_map.get("Сладкое", {}).get("Конфеты", [])
    assert confety[:2] == ["Акконд", "Коробочные"], (
        f"«Конфеты» должны начинаться с «Акконд», «Коробочные», но начинаются: {confety[:2]}"
    )
