...
updateBtn.onclick = async () => {
  ...
  try {
    ...
    await loadHistory();
    if (d.ok) {
      // Файлы ушли в обработку — очищаем список выбранных файлов (строки с «✕»),
      // чтобы он не висел до ручного закрытия. render([]) заодно отключает кнопку
      // (файлов больше нет) — поэтому ниже её НЕ включаем обратно.
      render([]);
      startPolling();
    } else {
      // Ошибка — оставляем список и возвращаем возможность повторить
