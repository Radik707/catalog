...
## Подушка безопасности (бэкап и откат)
- Перед каждым обновлением сервер копирует лист «Товары» → «Товары_BACKUP» (`scripts/sheet_tool.py backup`). Если бэкап не удался — обновление отменяется.
- После обновления, если товаров загрузилось резко меньше (< 50% прежнего), страница показывает предупреждение.
- Кнопка **«Откатить к прошлой версии»** возвращает каталог из «Товары_BACKUP» (`scripts/sheet_tool.py rollback`).
- Ручные команды (в venv, из scripts/): `python sheet_tool.py backup|rollback|count`.
