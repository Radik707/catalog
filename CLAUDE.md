...
- Для переноса отдельных товаров между категориями — `scripts/product_category_overrides.json`
  (подстрока названия → верная категория; применяется в upload.py ДО раскладки по разделам).
  Нужен, когда поставщик записал товар не в тот блок прайса (Киндер в макаронах и т.п.)
- **Переименования в прайсе и фото**: поставщик переставляет слова в названиях, из-за чего
  привязка фото (она держится на названии) рвалась пачками. Теперь это лечится само:
  `scripts/photo_rebind.py` опознаёт товар по отпечатку (значимые слова + граммовка) и
  переносит фото с прошлой версии каталога. Уверенные совпадения применяются и
  закрепляются, спорные — попадают в лог владельцу («Фото НЕ перенесено, нужен взгляд»).
  Правило безопасности: если у названий есть ВЗАИМНО различающиеся значимые слова
  (Эксклюзив ↔ Экстра, Цыганский ↔ Острый) — это разные товары, фото не переносится.
  Результат пишется в `photo_overrides_auto.json` / `photo_urls_auto.json` — это серверный
  runtime вне git (иначе git pull на сервере ломался бы). Ручные привязки из git главнее.
  Тесты: `python -m pytest scripts/test_photo_rebind.py -q`
